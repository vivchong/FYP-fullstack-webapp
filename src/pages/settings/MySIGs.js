  ...
  async function getPendingSIGForms() {
    try {
      const res = await fetch(
        'http://localhost:5000/settings/pending-sig-proposals',
        {
          method: 'POST',
              ...
              <Box key={pending.form_id} bg="white" boxShadow="sm" py={6} pl={6} pr={20}>
                <Flex gap={10}>
                  ...
                  <Flex flexDir="column" gap={2}>
                    ...
                    <Text>
                      Your proposal for creating "{<b>{pending.sig_name}</b>}"
                      is pending approval. Please wait for the SIG Head to get
                      back to you.
                    </Text>
                    <Flex>
                      <Spacer />
